		...
		$(function(){
						...
						top.$.jBox.confirm("确认导出？", "操作提示", function(v, h, f) {
							if (v == "ok") {
								$("#searchForm").attr('action',
										"${ctx}/mifi/useRecordStat/export2");
								$("#searchForm").submit();
								$("#searchForm").attr('action',
			...
			$('#btnSubmit').click(function(){
				...
				if(begin == "" ||  end == ""){
				 ...
				 $.ajax({
						type: 'post',
						url: '${ctx}/mifi/useRecordStat/statByYear',
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li  class="active"><a href="${ctx}/mifi/useRecordStat/year">按年统计</a></li>
	...
	<form:form id="searchForm"  class="breadcrumb form-search">
		...
		<input id="beginDate" name="beginDate"
				...
				class="input-small Wdate required" value="${year}"
				onFocus="var endDate = $dp.$('endDate');  WdatePicker({skin:'twoer',dateFmt:'yyyy' , onpicked:function(){endDate.focus();},minDate:'2015',maxDate:'{%y}'});" />&nbsp;到
		<input id="endDate" name="endDate"
				type="text" readonly="readonly" maxlength="20"
				class="input-small Wdate required" value="${year}"
				onFocus="WdatePicker({skin:'twoer',dateFmt:'yyyy', minDate:'#F{$dp.$D(\'beginDate\')}', maxDate:'{%y}'});" />		
		    ...
		    require(
		        ...
		        function(ec) {
		            ...
		            function eConsole(param) {
		            	...
		            	window.location.href = "${ctx}/mifi/useRecordStat/month?year="+orderValue + '&countryCode='+countryCode;
 		            }
