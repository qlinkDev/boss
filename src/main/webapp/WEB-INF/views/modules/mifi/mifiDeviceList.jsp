	...
	$(document).ready(
			function() {
							...
							top.$.jBox.confirm("确认要导出设备数据吗？", "系统提示", function(
								...
								if (v == "ok") {
									$("#searchForm").attr("action",
											"${ctx}/mifi/mifiDevice/export")
											.submit();
									$("#searchForm").attr("action",
											"${ctx}/mifi/mifiDevice/list");
								}
							}, {
								buttonsFocus : 1
							});
				...
				$("#btnSync").click(function() {
					top.$.jBox.confirm("确认要同步设备数据吗？", "系统提示", function(v, h, f) {
						if (v == "ok") {
							if(!!top.$.jBox.tip.mess){
						    	top.$.jBox.tip.mess='';
						    }
							top.$.jBox.tip("正在同步设备数据...", 'loading');
						    $("#searchForm").attr("action","${ctx}/mifi/mifiDevice/sync").submit();
...
<body>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/mifi/mifiDevice/import" method="post"
			...
			<br /> <input id="btnImportSubmit" class="btn btn-primary"
				...
				href="${ctx}/mifi/mifiDevice/import/template">下载模板</a>
		</form>
	</div>
	...
	<form:form id="searchForm" action="${ctx}/mifi/mifiDevice/list" method="post" class="breadcrumb form-search">
		...
		<div>
			<label>设备序列号：</label>
			<input id="sn" name="sn" type="text" maxlength="20" class="input-small" value="${sn}" /> 
			<label>设备ICCID：</label>
			<input id="iccId" name="iccId" type="text" maxlength="50" class="input-medium" value="${iccId}" /> 
			<label>归属类型：</label>
			<select id="ownerType" name="ownerType" class="input-small">
				<option value="">--请选择--</option>
				<c:forEach
					items="${fns:getDictList('mifi_owner_type')}"
					var="ownerTypeValue">
					<option value="${ownerTypeValue.value}"
						<c:if test="${ownerTypeValue.value==ownerType}">selected</c:if>>${ownerTypeValue.label}</option>
				</c:forEach>
			</select>
			<label>代理商：</label>
			<select id="sourceType" name="sourceType" class="input-small">
				...
				<c:forEach items="${fns:getListByTableAndWhere('om_channel','channel_name_en','channel_name',' and del_flag = 0 ')}" var="sourceTypeValue">
					<option value="${sourceTypeValue.channel_name_en}"
						<c:if test="${sourceTypeValue.channel_name_en==sourceType}">selected</c:if>>${sourceTypeValue.channel_name}
					</option>
				</c:forEach>
			</select> 
			<label>批次号：</label><input id="bath" name="bath" type="text" maxlength="50" class="input-small required" value="${bath}" />
			<label>归属地：</label>
			...
			<shiro:hasPermission name="mifi:mifiDevice:import">
			&nbsp;<input id="btnImport" class="btn btn-primary" type="button" value="导入"/>
			</shiro:hasPermission>
			
			<shiro:hasPermission name="mifi:mifiDevice:sync">
		...
		<tbody>
			<c:forEach items="${page.list}" var="a">
				<tr>
					...
					<td>${a.source_type}|${fns:getLabelByTable('om_channel','channel_name_en','channel_name',a.source_type)}</td>
					<td>${a.datainfo}</td>
					...
					<td>${a.in_user}</td>
					<td>${a.test_ip}</td>
					<td>${a.test_update_ip}</td>
					<td>${(empty a.softsim_type || a.softsim_type==0) ? '否' : '是'}</td>
					<td>${(empty a.web_portal_flag || a.web_portal_flag==0) ? '否' : '是'}</td>
					<td>${fns:getDictLabel(a.order_type, 'yes_no', '否')}</td>
					<td>${a.flow}</td>
					<td>${a.flow_used}</td>
					<td>${a.start_time}</td>
					<td>${a.end_time}</td>
