...
<body>
	...
	<form:form id="searchForm" modelAttribute="deviceBootCondition" action="${ctx}/mifi/deviceBoot/" method="post" class="breadcrumb form-search">
		...
		<label>开机日期：</label>
		<form:input id="geCreateDate" path="geCreateDate" type="text" readonly="true" maxlength="20" class="input-medium Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:true});"/>
		到
		<form:input id="leCreateDate" path="leCreateDate" type="text" readonly="true" maxlength="20" class="input-medium Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',isShowClear:true});"/>
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${page.list}" var="boot">
				<tr>
					...
					<td>${fns:getLabelByTableAndWhere('om_channel','channel_name_en','channel_name',' and del_flag = 0 ', boot.sourceType)}</td>
					<td>${boot.mac}</td>
					<td>${boot.mcc}</td>
