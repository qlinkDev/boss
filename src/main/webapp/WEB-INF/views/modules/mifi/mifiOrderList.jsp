...
<head>
	...
	<script type="text/javascript">
		...
		function detail(orderNo){
			$.jBox("get:${ctx}/mifi/mifiOrderList/detail?orderNo="+orderNo,{title:"订单详情", width:'auto', persistent:true,showScrolling:true, buttons: { '关闭': 'ok' }});
		}
	</script>
</head>
<body>
	...
	<form:form id="searchForm" action="${ctx}/mifi/mifiOrderList/"
		...
		<div>
			...
			<label>目的区域：</label>
			<select id="allowedRegion" name="allowedRegion"  class="input-small">
				<option value="">--请选择--</option>
				<c:forEach var="item"  items="${regionList}">
					<option value="${item.code}" <c:if test="${item.code == allowedRegion}">selected</c:if>>${item.name}</option>
				</c:forEach>
			</select>
			<label>订单进行中：</label> 
			...
			<select id="orderStatus" name="orderStatus" class="input-small">
				...
				<c:forEach items="${fns:getDictList('mifi_order_status')}"
					...
					<option value="${orderStockStatus.value}" <c:if test="${orderStockStatus.value==orderStatus}">selected</c:if>>${orderStockStatus.label}</option>
				</c:forEach>
			</select>
			...
			<select id="stockStatus" name="stockStatus" class="input-small">
				<option value="">--请选择--</option>
				<c:forEach items="${fns:getDictList('order_stock_status')}" var="orderStockStatus">
					<option value="${orderStockStatus.value}" <c:if test="${orderStockStatus.value==stockStatus}">selected</c:if>>${orderStockStatus.label}</option>
				</c:forEach>
			</select> 
			<label>下单时间：</label> 
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed" style="width:100%">
			...
			<tbody>
				<c:forEach items="${page.list}" var="a">
					<tr>
						<td><a href="javascript:void(0);" onclick="detail('${a.out_order_id}')">${a.out_order_id}</a></td>
						<td>${a.order_type }|${fns:getDictLabel(a.order_type, 'mifi_order_type', '未知类型')}</td>
						...
						<td>
							<c:if test="${a.order_status!='11' }">
								<c:if test="${(a.stock_status!='1') && (a.order_status=='1'||a.order_status=='2')}">
									<shiro:hasPermission name="mifi:mifiOrder:stock">
									<a href="${ctx}/mifi/mifiOrderList/stockSimCard?order_id=${a.order_id}&startDate=${startDate}&endDate=${endDate}&sourceType=${sourceType}&stockStatus=${stockStatus}&outOrderId=${outOrderId}&orderStatus=${orderStatus}" onclick="return confirmx('确认[已备SIM卡]？', this.href)">已备卡</a>
									</shiro:hasPermission>
								</c:if>
