...
<body>
	...
	<form:form id="searchForm" modelAttribute="workOrderCondition" action="${ctx}/mifi/wOrder/list" method="post" class="breadcrumb form-search">
		...
		<label>渠道商编号：</label><form:input id="channelSn" path="channelSn" htmlEscape="false" maxlength="50" class="input-small"/>
		<label>问题类型：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="workOrder">
			<tr>
				...
				<td>${workOrder.channelSn}</td>
				<td width="35%;">${workOrder.problemDesc}</td>
				<td>${fns:getDictLabel(workOrder.problemType,'work_order_problem_type','')}</td>
				<td width="2%;">${fns:getDictLabel(workOrder.level,'work_order_level','')}</td>
