...
<body>
	...
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<label>异常：</label><input id="exception" name="exception" type="text" maxlength="50" class="input-small" value="${exception}"/>
			&nbsp;
			...
			<label for="exception">异常类型：
				<select id="type" name="type" class="input-small">
						<option value="">请选择</option>
						<c:forEach var="item" items="${fns:getDictList('sys_log_type') }">
							<option value="${item.value }" ${item.value == type ? 'selected':'' }>${item.label }</option>
						</c:forEach>
				</select>
			</label>
			...
			<c:forEach items="${page.list}" var="log">
				...
				<c:choose>
					<c:when test="${log.type == 4 }">
						<tr>
							<td colspan="8">${fn:replace(fns:escapeHtml(log.exception), strEnter, '<br/>')}!!!!</td>
						</tr>
					</c:when>
					<c:otherwise>
						<tr>
