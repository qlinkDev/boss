			...
			if (user!=null){
				...
				if (startupPath!=null){
					...
					if (ss.length == 2) {
						...
						String realPath = request.getSession().getServletContext().getRealPath(StringUtils.EMPTY);
						if (null == realPath) {
							realPath = this.getServletConfig().getServletContext().getRealPath(StringUtils.EMPTY);
						}
						realPath += path;
