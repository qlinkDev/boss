...
import com.uu.common.security.Digests;
...
import com.uu.modules.sys.dao.MenuDao;
import com.uu.modules.sys.dao.RoleDao;
import com.uu.modules.sys.dao.UserDao;
...
import com.uu.modules.sys.security.SystemAuthorizingRealm;
...
public class SystemService extends BaseService  {
	...
	//	DetachedCriteria a=dc.createAlias("sys_user_role","r");
		...
		if (StringUtils.isNotEmpty(user.getChannelNameEn())){
			dc.add(Restrictions.like("channelNameEn", "%" + user.getChannelNameEn() + "%"));
		}
	...
	public Page<HashMap> findUserlist(Page<HashMap> page, User user) {
		User currentUser = UserUtils.getUser();
		StringBuffer sb = new StringBuffer("SELECT s.id, s.login_name, s.channel_name, s.name, s.mobile, s.phone, o.`name` companyName, e.`name` allRoles FROM sys_user s, sys_user_role r, sys_office o, sys_role e WHERE s.office_id = o.id AND s.id = r.user_id AND e.id =r.role_id AND s.del_flag = '0' ");
		if(user.getOffice()!=null){
		if (StringUtils.isNotEmpty(user.getOffice().getId())){
			sb.append(" and s.office_id  = '"+ user.getOffice().getId() + "'");
		}}
		if(user.getCompany()!=null){
			if (StringUtils.isNotEmpty(user.getCompany().getId())){
				sb.append(" and s.company_id  = '"+ user.getCompany().getId() + "'");
			}
		}
		if (StringUtils.isNotEmpty(user.getLoginName())){
			sb.append(" and s.login_name  like '"+"%" + user.getLoginName()  +"%"+ "'");
		}
		if (StringUtils.isNotEmpty(user.getName())){
			sb.append(" and s.name  like '"+"%" + user.getName()  +"%"+ "'");
		}
		if (StringUtils.isNotEmpty(user.getChannelNameEn())){
			sb.append(" and s.channel_name_en  = '"+  user.getChannelNameEn()  + "'");
		}
		if(user.getRoleList().size()>0 ){
			sb.append("and s.id = r.user_id  AND  r.role_id= '" + user.getRoleList().get(0).getId()+ "'");
		}
		if (!currentUser.isAdmin()){
			sb.append(" AND r.role_id!='1' ");
		}
		if (!StringUtils.isNotEmpty(page.getOrderBy())){
			sb.append(" order  by s.create_date desc ");
		}
		return userDao.findBySql(page, sb.toString(), null, Map.class);
	}
	/**
	 * 导出
	 * @Description 
	 ...
	 * @return List<User>  
	 * @author wangsai
	 * @date 2017年3月17日 下午2:19:10
	 */
	public  List<User> findUserListExport(User user) throws ParseException{
		StringBuffer sb = new StringBuffer("SELECT  o.`name` companyName,s.channel_name,s.login_name,s.name,s.no,s.email,s.phone,s.mobile ,s.user_type ,s.create_date,s.login_ip,s.login_date,e.`name` allRoles,s.remarks FROM sys_user s, sys_user_role r, sys_office o, sys_role e WHERE s.office_id = o.id AND s.id = r.user_id AND e.id =r.role_id AND s.del_flag = '0' ");
		if (StringUtils.isNotEmpty(user.getLoginName())){
			sb.append(" and s.login_name  like '"+"%" + user.getLoginName()  +"%"+ "'");
		}
		if (StringUtils.isNotEmpty(user.getName())){
		...
		if(user.getRoleList().size()>0 ){
			...
			sb.append(" GROUP  by s.create_date desc ");
			List<String[]> listObjArr = userDao.findBySql(sb.toString());
			...
			if (listObjArr!=null && listObjArr.size()>0) {
				...
				for (Object[] objs : listObjArr) {
					...
					office.setName(ObjectUtils.toString(objs[0]));
					users.setCompany(office);
					users.setOffice(office);
					users.setChannelName(ObjectUtils.toString(objs[1]));
					users.setLoginName(ObjectUtils.toString(objs[2]));
					users.setName(ObjectUtils.toString(objs[3]));
					users.setNo(ObjectUtils.toString(objs[4]));
					users.setEmail(ObjectUtils.toString(objs[5]));
					users.setPhone(ObjectUtils.toString(objs[6]));
					users.setMobile(ObjectUtils.toString(objs[7]));
					users.setUserType(ObjectUtils.toString(objs[8]));
					users.setCreateDate((Date) objs[9]);
					users.setLoginIp(ObjectUtils.toString(objs[10]));
					users.setLoginDate((Date) objs[11]);
					users.setChannelNameEn(ObjectUtils.toString(objs[12]));
					users.setRemarks(ObjectUtils.toString(objs[13]));
					list.add(users);
					users = null;
				}
			}
	...
	public User getUserByChannelNameEn(String channelNameEn) {
		return userDao.findByChannelNameEn(channelNameEn);
	}
	...
	//@Transactional(readOnly = false)
	public Boolean saveAndOutUserInRole(Role role, String userId) {
	...
	//@Transactional(readOnly = false)
	public User saveAndAssignUserToRole(Role role, String userId) {
		User user = userDao.get(userId);
		List<String> roleIds = user.getRoleIdList();
		if (roleIds.contains(role.getId())) {
			return null;
	 ...
	 */
	//@Transactional(readOnly = false)
	public void saveAndSynToActiviti()  {
		try{
