/**
 */
package main.java.com.qlink.modules.oa.service;
...
import com.uu.modules.oa.dao.LeaveDao;
...
public class LeaveService extends BaseService {
	...
	//部门领导审批
	//@Transactional(readOnly = false)
	public void saveAndDeptLeaderAudit(Leave leave) {
	...
	//人事审批
	//@Transactional(readOnly = false)
	public void saveAndHrAudit(Leave leave) {
		WorkflowUtils.claim(leave.getProcessInstanceId());
		Task task = taskService.createTaskQuery().processInstanceId(leave.getProcessInstanceId()).singleResult();
		//添加批注
		taskService.addComment(task.getId(), leave.getProcessInstanceId(), leave.getAuditRemarks());
	...
	//调整申请
	//@Transactional(readOnly = false)
	public void saveAndModifyApply(Leave leave) {
		WorkflowUtils.claim(leave.getProcessInstanceId());
		Task task = taskService.createTaskQuery().processInstanceId(leave.getProcessInstanceId()).singleResult();
		Map<String, Object> map = Maps.newHashMap();
	...
	//销假
	//@Transactional(readOnly = false)
	public void saveAndReportBack(Leave leave) {
		WorkflowUtils.claim(leave.getProcessInstanceId());
		Task task = taskService.createTaskQuery().processInstanceId(leave.getProcessInstanceId()).singleResult();
		//完成任务
